...
import {
  ...
  Edit3, Send, Eye, Shield, Lock, UserX, X, Calendar, BookOpen, DollarSign,
} from "lucide-react";

const categoryStyles: Record<string, { bg: string; text: string }> = {
  evento: { bg: "bg-purple-100", text: "text-purple-700" },
  regras: { bg: "bg-amber-100", text: "text-amber-700" },
  financeiro: { bg: "bg-emerald-100", text: "text-emerald-700" },
};

const categoryIcons: Record<string, React.ReactNode> = {
  evento: <Calendar className="w-5 h-5 text-purple-600" />,
  regras: <BookOpen className="w-5 h-5 text-amber-600" />,
  financeiro: <DollarSign className="w-5 h-5 text-emerald-600" />,
};

export default function AssociacoesLandingPage() {
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
            <div className="flex-1 text-center lg:text-left">
              ...
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-foreground mb-6">
                Avisos importantes para associados,{" "}
                <span className="text-primary">em um só lugar.</span>
              </h1>
              <p className="text-lg md:text-xl text-muted-foreground max-w-xl mx-auto lg:mx-0 mb-8 leading-relaxed">
                Timeline oficial + lembretes por WhatsApp e e-mail.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-6">
                <Button asChild size="lg" className="touch-target text-base font-semibold px-8">
                  <Link to="/auth/signup/community">Criar canal oficial do clube<ArrowRight className="w-5 h-5 ml-2" /></Link>
                </Button>
              </div>
              ...
              <div className="relative">
                <div className="bg-card rounded-3xl shadow-2xl border border-border p-4 mx-auto max-w-sm">
                  <div className="bg-muted rounded-2xl p-4 space-y-4">
                    <div className="text-center pb-3 border-b border-border">
                      <h3 className="font-display font-bold text-lg">Clube Atlântico</h3>
                      <p className="text-sm text-muted-foreground">Canal Oficial</p>
                    </div>
                    {[
                      { type: "evento", title: "Evento do Fim de Semana", subtitle: "Sábado, a partir das 10h" },
                      { type: "regras", title: "Mudança de Regulamento", subtitle: "Nova versão em vigor" },
                      { type: "financeiro", title: "Taxa Semestral", subtitle: "Vencimento: 15/03" },
                    ].map((a, i) => {
                      const s = { evento: { bg: "bg-purple-100", icon: "text-purple-600", badge: "bg-purple-100 text-purple-700", label: "Evento" }, regras: { bg: "bg-amber-100", icon: "text-amber-600", badge: "bg-amber-100 text-amber-700", label: "Regras" }, financeiro: { bg: "bg-emerald-100", icon: "text-emerald-600", badge: "bg-emerald-100 text-emerald-700", label: "Financeiro" } }[a.type]!;
                      return (
                ...
                <div className="absolute -right-4 top-1/4 bg-card rounded-xl shadow-lg border border-border p-3 max-w-[200px] hidden md:block">
                  <div className="flex items-center gap-2">
                    ...
                    <div><p className="text-xs font-medium">WhatsApp enviado!</p><p className="text-xs text-muted-foreground">120 associados</p></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-16 md:py-24 bg-muted/30">
        <div className="container px-4 mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">O presidente conhece bem{" "}<span className="text-primary">esses problemas</span></h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {[
              { icon: MessageSquareOff, title: "Avisos de eventos se perdem", desc: "Convites e programações somem em grupos lotados de mensagens." },
              { icon: EyeOff, title: "Associados dizem que não viram", desc: "Sem registro, não há como comprovar que a informação foi enviada." },
              { icon: RefreshCw, title: "Regras não ficam claras", desc: "Mudanças de regulamento circulam de boca em boca e geram confusão." },
            ].map((item) => (
          ...
          <p className="text-center text-muted-foreground mt-10 max-w-lg mx-auto">A associação precisa de uma{" "}<span className="font-semibold text-foreground">fonte oficial de informação.</span></p>
        </div>
      </section>
        ...
        <div className="container px-4 mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              ...
              { num: "03", icon: Eye, title: "Associados consultam", desc: "A informação fica disponível na linha do tempo a qualquer momento." },
            ].map((step) => (
      ...
      <section className="py-16 md:py-24 bg-muted/30">
        <div className="container px-4 mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {[
              { title: "Eventos bem divulgados", desc: "Convites e programações ficam registrados e acessíveis para todos os associados." },
              { title: "Regulamento claro", desc: "Mudanças de regras ficam documentadas. Sem espaço para 'eu não sabia'." },
              { title: "Histórico completo", desc: "Todos os comunicados organizados por data e categoria. Fácil de consultar." },
              { title: "Menos reclamações", desc: "Quando a informação está disponível, o desgaste com associados diminui." },
            ].map((b) => (
      ...
      <section className="py-16 md:py-24">
        <div className="container px-4 mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <p className="text-lg text-muted-foreground">Veja como ficam os comunicados na linha do tempo do clube.</p>
          </div>
          <div className="max-w-2xl mx-auto space-y-4">
            {[
              { cat: "evento", label: "Evento", title: "Torneio de tênis do fim de semana", time: "Hoje, 09:00", content: "Inscrições abertas até sexta-feira. Confira as categorias e horários." },
              { cat: "regras", label: "Regras", title: "Mudança de regulamento da piscina", time: "Ontem, 15:00", content: "Novo horário de funcionamento e regras de uso a partir de março." },
              { cat: "financeiro", label: "Financeiro", title: "Taxa semestral disponível", time: "10/02", content: "O boleto da taxa semestral já está disponível. Vencimento: 15/03." },
            ].map((a) => {
        ...
        <div className="container px-4 mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            ...
            <div className="rounded-2xl border-2 border-primary bg-card p-8 shadow-md">
              ...
              <ul className="space-y-3 text-sm text-muted-foreground">
                {["Linha do tempo oficial do clube", "Categorizado por tipo de aviso", "Histórico completo e acessível", "Acesso sem login para associados"].map((t) => (
      ...
      <section className="py-16 md:py-24">
        <div className="container px-4 mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">Privacidade dos associados</h2>
            <p className="text-lg text-muted-foreground">Transparência em cada comunicação.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              { icon: Shield, title: "Apenas avisos oficiais", desc: "Contatos usados exclusivamente para enviar comunicados importantes do clube." },
              { icon: Lock, title: "Sem propaganda", desc: "Nenhum dado é compartilhado com terceiros. Sem envio de publicidade." },
              { icon: UserX, title: "Descadastro a qualquer momento", desc: "Qualquer associado pode optar por não receber mais notificações quando quiser." },
            ].map((item) => (
      ...
      <section className="py-16 md:py-24 bg-primary text-primary-foreground">
        <div className="container px-4 mx-auto text-center">
          <h2 className="font-display text-3xl md:text-4xl font-bold mb-6">Pronto para organizar a comunicação do seu clube?</h2>
