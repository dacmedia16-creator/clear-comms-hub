  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileError || !profile) {
        ...
        toast({
          title: "Conta criada",
          description: `Sua conta foi criada, mas houve um erro ao vincular. Entre em contato com o ${terms.manager.toLowerCase()}.`,
          variant: "default",
        });
        navigate("/dashboard");
        return;
      ...
      // 3. Link to organization with resident role (pending approval)
      ...
      if (roleError) {
        ...
        toast({
          ...
          description: formConfig.memberSuccessMessage(organization.name, terms.manager),
        });
      }
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardTitle className="font-display text-2xl transition-all duration-300">
              {organization ? `Cadastro de ${terms.member}` : formConfig.memberTitle}
            </CardTitle>
            <CardDescription className="transition-all duration-300">
              {organization 
                ? `Entre com seus dados para se cadastrar em ${organization.name}`
                : formConfig.memberDescription
              }
            </CardDescription>
            {organization && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {organization ? `Solicite ao ${terms.manager.toLowerCase()}` : formConfig.codeDescription}
                </p>
              </div>
